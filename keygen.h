/*
  KeyMaker.c, version 4.11 (14Jun2004)

  Stand-alone key generator source code for the Armadillo Software Protection
  System, by the Silicon Realms Toolworks. Copyright 1999-2004 by Chad Nelson
...
CRC32 crc32(const char *s, unsigned long length, CRC32 crc)
{
    ...
    if (!table32)
    {
        ...
        for (x=0; x<alphabet; ++x)
        {
            ...
            for (b=0; b<alphabits; ++b) if (r&topbit) r=(r<<1)^poly32;
                else r<<=1;
...
const char *CreateKey(const char *regname, const char *keytext, unsigned short otherinfo, unsigned long hardwareID)
{
    ...
    unsigned long k[2];
    unsigned symkey;
    sscanf(TPoDT_SYM, "%08X", &symkey);
    char cooked[512];

    if (*regname==0) return "NoNameError";

    CookText(cooked, keytext);
    k[0]=symkey^hardwareID^HWIDXOR;
...
void BigInt_FromHexString(const char *string, BigInt target)
{
    ///Greetings from Mr. eXoDia!
    char bytes[2]="";
    char temp1[256]="";
    char temp2[256]="";
    char temp3[256]="";
...
void ECC_Initialize(EC_PARAMETER *Base, EC_KEYPAIR *Signer, unsigned long basepointinit, const char *_rndinitstring)
{
    ...
    InitRandomGenerator(i[0]^i[1]^i[2]^i[3]); ///symkey
...
/*void GCDTest(void) {
	...
	// Make pvt
	GenerateKeyNumberFromString(pvtkeystring, p, &pvt, KS_SHORTV3, 0);
...
static int MakeSignature(unsigned char *keybytes, int *keylength, char *pvtkeystring, char *name_encryptkey, int level)
{
    ...
    */
    ...
    ///Inject PVT here!
    BigInt_FromHexString(TPoDT_PVT, pvt);
    BigInt_PowerModulus(pub, pvt, p, y);
    ///Inject Y here!
    BigInt_FromHexString(TPoDT_Y, y);
    ...
    while (1)
    {
        ...
        section replaces these two lines:
        	...
        	BigInt_PowerModulus(pvt, temp, p, k);
        */
        sprintf(tmp, "%u%s", (unsigned int)++ksource, pvtkeystring);
        BigInt_Set(temp, 0);
        for (x=0; x<5; ++x)
        {
...
const char *CreateSignedKey(int level, char *encryption_template, char *_name_to_make_key_for, unsigned long hardwareID, unsigned short otherinfo1, unsigned short otherinfo2, unsigned short otherinfo3, unsigned short otherinfo4, unsigned short otherinfo5)
{
    static char retval[512]="";
    char name_to_make_key_for[512], temp[512], *cc, *cc2, *shortv3digits=(char*)"0123456789ABCDEFGHJKMNPQRTUVWXYZ";
    ...
    unsigned int inject_symkey=0;
    sscanf(TPoDT_SYM, "%X", &inject_symkey);
    symmetrickey=inject_symkey^hardwareID;
...
{
    return CreateSignedKey(level, encryption_template, name_to_make_key_for, hardwareID, otherinfo1, otherinfo2, otherinfo3, otherinfo4, otherinfo5);
};
...
{
    return CreateSignedKey(level+10, encryption_template, name_to_make_key_for, hardwareID, otherinfo1, otherinfo2, otherinfo3, otherinfo4, otherinfo5);
};
...
{
    return CreateSignedKey(level+20, encryption_template, name_to_make_key_for, hardwareID, otherinfo1, otherinfo2, otherinfo3, otherinfo4, otherinfo5);
};
...
char RetrieveKeyInfo(int level, const char *encrypt_template, const char *name, unsigned long hardwareID, const char *origkey, const char *uninstallkey, struct KeyInformation *keyinfo)
{
    ...
    {
        char keybytes[512], signature[512], *kb=keybytes, *k1, *k2;
